...
### Added
- **Collection Runner**: Run all requests in a collection or folder sequentially via context menu ("Run Collection" / "Run Folder") or command palette (`LiteClient: Run Collection`).
- **Runner Results Panel**: Real-time progress bar, streaming results table with pass/fail status, colored method badges, and duration per request. Expandable rows show full response body (auto-formatted JSON), response headers, test results, console output, and script errors.
- **Variable Chaining**: Variables set via `pm.environment.set()`, `pm.globals.set()`, or `pm.collectionVariables.set()` in one request's scripts carry forward to subsequent requests in the run.
- **Cookie Persistence**: Cookies set during the run carry forward across requests, matching Postman runner behavior.
- **Run History**: Each request executed by the runner is saved to history with its collection source, appearing in the sidebar history panel.
